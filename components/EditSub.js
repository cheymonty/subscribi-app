...
export const EditSub = React.forwardRef(({oldSub}, ref) => {
    const [name, setName] = useState(oldSub.name)
    const [cost, setCost] = useState(oldSub.cost)
    const [startDate, setStartDate] = useState(oldSub.startDay)
    const [startDatePicker, setStartDatePicker] = useState(false)
    const [durationPicker, setDurationPicker] = useState(false)
    const [duration, setDuration] = useState(oldSub.duration)
    ...
    function submit() {
        ...
        //TODO: need to delete old notification
        if (endDate > new Date())
    ...
    return (
        <ActionSheet ref={ref} bounceOnOpen gestureEnabled onClose={reset} openAnimationSpeed={15} containerStyle={{backgroundColor: theme.modal}}>
            <Title style={styles(theme).sheetTitle}>{`Edit ${name}`}</Title>
